...
from model_organic import DataModel
...
def to_db(output, vid):
    for row in output:
        data_model = DataModel(organic_sessions=row['ga:sessions'], date=str(pd.to_datetime(row['ga:date'])), bounce_rate=row['ga:bounceRate'], view_id=vid)
        data_model.save()
    return
...
def queried_data():
    ...
    for date in dates:
        #row = {}
        dm =  DataModel.select(fn.SUM(DataModel.organic_sessions).alias("organic_sessions"), DataModel.date, fn.SUM(DataModel.bounce_rate).alias("bounce_rate")).where(DataModel.date == date).where(DataModel.view_id.in_(VIEW_ID))
        for d in dm:
            results.append({'date': d.date, 'organic_sessions': d.organic_sessions, 'sessions_avg': d.organic_sessions//len(VIEW_ID), 'bounce_rate': d.bounce_rate//len(VIEW_ID)})
    return results
...
if __name__ == '__main__':
    ...
    df = to_dataframe_v2()
    df.to_csv('{}/sheets/{}.csv'.format(BASE_DIR, 'sammy_organic', vid), index=False)
    time.sleep(0.5)
